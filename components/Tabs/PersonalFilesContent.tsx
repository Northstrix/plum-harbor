...
import FileDownloader from '@/components/File-Related/PersonalFileDownloader';
import BauhausFileCard from '@/components/File-Related/BauhausFileCard/BauhausFileCard'
import { fileTypeClassification } from '@/app/lib/fileTypeClassification'
import BauhausPopUpModal from '@/components/ui/BauhausPopUpModal/BauhausPopUpModal';
...
const PersonalFilesContent: React.FC = () => {
    ...
    const [fileTagForDownloader, setFileTagForDownloader] = useState<FileTagForDownloader | null>(null);
    ...
    const [showBauhausPopUpModal, setShowBauhausPopUpModal] = useState(false);
    const [bauhausPopUpModalTitle, setBauhausPopUpModalTitle] = useState('');
    const [bauhausPopUpModalDescription, setBauhausPopUpModalDescription] = useState('');
    const [bauhausPopUpModalDescription2, setBauhausPopUpModalDescription2] = useState('');
    const [bauhausPopUpModalProgress, setBauhausPopUpModalProgress] = useState(0);
    ...
    useEffect(() => {
      ...
      setBauhausPopUpModalTitle(t('fetching-the-metadata-of-your-files'));
      setBauhausPopUpModalDescription(t('please_wait'));
      setBauhausPopUpModalDescription2(" ");
      setBauhausPopUpModalProgress(0);
      setShowBauhausPopUpModal(true);
      ...
      singleton.runEffect(async () => {
        try {
          ...
          const filesRef = collection(db, `data/${user.email}/private/personalFiles/metadata`);
          ...
          for (const doc of querySnapshot.docs) {
            ...
            const title = t('processing_file', {
              fileIndex: fileIndex,
              totalFiles: querySnapshot.docs.length,
            });
            setBauhausPopUpModalTitle(title);
            setBauhausPopUpModalDescription(t('decrypting-the-file-metadata'));
            setBauhausPopUpModalDescription2(t('please_wait'));
            setBauhausPopUpModalProgress((fileIndex/querySnapshot.docs.length) * 100);
              ...
              let decryptedDescriptionPaddingValidity = false;
                ...
                if (data.encryptedFilename && data.encryptedFilename.trim() !== '') {
                  ...
                  if (decoded.length % 16 === 0) {
                    const [decryptedFileNameArray, integrity, paddingValidity] = await decryptStringWithTwoCiphersCBC(decoded);
                ...
                if (data.encryptedDescription && data.encryptedDescription.trim() !== '') {
                  if (data.encryptedDescription !== "Tm8gZGVzY3JpcHRpb24u") {
                    ...
                    if (decoded.length % 16 === 0) {
                      const [decryptedFileDescriptionArray, descrIntegrity, descrPaddingValidity] = await decryptStringWithTwoCiphersCBC(decoded);
          ...
          setShowBauhausPopUpModal(false);
    ...
    const decryptStringWithTwoCiphersCBC = async (
      ...
      const new_iterations: number = Math.round(109 + (iterations/11));
      const derivedKey = await deriveBytesUsingArgon2id(masterKey, salt, new_iterations, 224);
    ...
    const calculateColumns = () => {
        ...
        const itemWidth = 336; // Width of each item
        ...
        const nonEmptyFiles = Array.from(files).filter(file => {
          ...
          for (let i = 0; i < nonEmptyFiles.length; i++) {
            ...
            try {
              ...
              await new Promise(resolve => setTimeout(resolve, 50));
              setProcessingStep(t('deriving-encryption-key-using-argon2id'));
              await new Promise(resolve => setTimeout(resolve, 50));
              
              const [encryptedData, tag, recordKey] = await encryptFileWithTwoCiphersCBC(fileBytes);
              ...
              setProcessingStepDescription(t('please_wait'));
              const encryptedFilename = await encryptDataWithTwoCiphersCBC(filenameBytes);
              let encryptedDescription;
              if (new TextDecoder().decode(description) !== 'No description.') {
                  toggleProgressAnimation(true);
                  setProcessingStep(t('encrypting-description'));
                  encryptedDescription = await encryptDataWithTwoCiphersCBC(description);
              } else {
                  encryptedDescription = new TextEncoder().encode('No description.');
              }

              setProcessingStep(t('encrypting-record-tag'));
              setProcessingStepDescription(t('please_wait'));
        ...
        const uploadFile = async (
          ...
          try {
            ...
            while (!isUnique) {
              ...
              const docRef = doc(db, `data/${user.email}/private/personalFiles/metadata`, uniqueId);
            ...
            for (let i = 0; i < totalChunks; i++) {
              ...
              await setDoc(doc(db, `data/${user.email}/private/personalFiles/${uniqueId}`, `${i}`), {
                data: chunkB64
              });
            ...
            await setDoc(doc(db, `data/${user.email}/private/personalFiles/metadata`, uniqueId), recordData);
    ...
    const handleMoreInfoClick = async (id: string) => {
      const container = containers.find(helper => helper.id === id);
      const containerHelper = containerHelpers.find(ch => ch.id === id);
      ...
      popup.innerHTML = `
        <div class="popup-content" id="popup-card">
          ...
          <p><strong>Size:</strong> ${container.fileSize}</p>
          <p><strong>Encrypted size:</strong> ${generateFileSizeString(containerHelper.encryptedLength)}</p>
    ...
    const handleDownload = async (id: string) => {
      ...
      if (!user) {
        ...
        if (container && containerHelper) {
          setFileTagForDownloader({
            container,
            containerHelper
          });
          downloaderRef.current?.initiateDownload?.();
        } else {
          console.error('Container or ContainerHelper not found for id:', id);
        }
      }
    };
    ...
    const handleDelete = async (id: string) => {
      ...
      if (result.isConfirmed) {
        ...
        await deleteRecord(id, container!.title);
      }
    ...
    const deleteCollection = async (containerId: string, totalChunks: number) => {
      ...
      for (let chunkNumber = 0; chunkNumber < totalChunks; chunkNumber++) {
        const chunkRef = doc(db, `data/${auth.currentUser?.email}/private/personalFiles/${containerId}`, `${chunkNumber}`);
      ...
      if (selectedRecordId) {
        try {
          ...
          if (typeof encryptedLength !== 'number') {
            ...
            const collectionRef = collection(db, `data/${user.email}/private/personalFiles/${containerId}`);
            ...
            const chunkRef = doc(db, `data/${auth.currentUser?.email}/private/personalFiles/metadata`, `${containerId}`);
            //console.log(`Preparing to delete chunk document: ${chunkRef.path}`);
            batch.delete(chunkRef);
            await batch.commit();
    ...
    return (
        <div className="flex flex-col items-center">
          <span style={{
            ...
            fontSize: '24px',
            fontWeight: 'bold',
            textAlign: i18n.language === "he" ? 'right' : 'left', // Align text based on language
          }}>
            {t('personal-files-tab')} {/* Translated text */}
          </span>
          {descriptionPopup}
          {showProcessingPopup && (
          ...
          `}</style>
            ...
            <div 
                style={{ 
                    marginTop: "2.25rem", 
                    width: `${Math.max(336, columns * 336 + (columns - 1) * 36)}px`
                }}
            >
            ...
            <BentoGrid className={`mt-9`} style={{ gridTemplateColumns: `repeat(${columns}, minmax(0,1fr))` }}>
                {containers.map(file => (
                  <BentoGridItem key={file.id}>
                    <BauhausFileCard
                        id={file.id}
                        accentColor={fileTypeClassification.find(type =>
                          type.extensions.some(ext => file.title.toLowerCase().endsWith(ext))
                      )?.color || '#3D4785'}
                        topInscription={file.fileSize}
                        fileName={file.title}
                        subMainText={file.description === "No description." ? t('no-description') : file.description}
                        filledButtonInscription={t('download')}
                        outlinedButtonInscription={t('delete')}
                        onFilledButtonClick={handleDownload}
                        onOutlinedButtonClick={handleDelete}
                        filledChronicleButtonHoverColor={lightenColor(
                          fileTypeClassification.find(type =>
                              type.extensions.some(ext => file.title.toLowerCase().endsWith(ext))
                          )?.color || '#3D4785', 0.32
                        )}
                        oulinedChronicleButtonHoverColor="var(--deleteOutlinedChronicleButtonColor)"
                        onTitleClick={handleMoreInfoClick}
                        onDescriptionClick={handleMoreInfoClick}
            ...
            <div className="pt-10 pb-5 text-center"> {/* Padding and margin for spacing */}
              <span 
            ...
            {showBauhausPopUpModal && (
              <BauhausPopUpModal
                mainInscription={bauhausPopUpModalTitle}
                secondaryInscription={bauhausPopUpModalDescription}
                secondaryInscription2={bauhausPopUpModalDescription2}
                progress={bauhausPopUpModalProgress}
              />
            )}
        </div>
    );
...
export default PersonalFilesContent;