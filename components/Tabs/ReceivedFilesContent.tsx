...
import { silentlyDecryptDataWithTwoCiphersCBC } from '@/app/cryptographicPrimitives/twoCiphersSilentMode';
...
export default function ReceivedFilesContent() {
...
const extractRecordsFromFirebase = () => {
  ...
  const fetchLogins = async () => {
    ...
    try {
      ...
      const keyRef = doc(db, `data/${user.email}/private/encrypted/keyring`, 'mlkem-private-key'); // Reference to the document named 'mlkem-public-key'
      const keyDoc = await getDoc(keyRef); // Fetch the document snapshot
      //console.log(`data/${recipientEmail}/public/mlkem-public-key`);
      if (!keyDoc.exists()) {
        showTwoLinedPopup('Error', "Can't access the ML-KEM private key. Please refresh the page and try logging in again.");
        await new Promise(() => {});
        throw new Error('User not authenticated');
      } else {
          const keyData = keyDoc.data();
          if (keyData && 'privateKey' in keyData) {
            const encrypted_ml_kem_private_key = base64ToUint8Array(keyData.privateKey);
            if (encrypted_ml_kem_private_key.length % 16 === 0 && encrypted_ml_kem_private_key.length > 1){
              const cut_iterations = parseInt((iterations / 10).toString(), 10);
              const [decrypted_private_key, privateKeyIntegrityFailure] = await silentlyDecryptDataWithTwoCiphersCBC(encrypted_ml_kem_private_key, masterKey, cut_iterations);
              if (privateKeyIntegrityFailure === true){
                showTwoLinedPopup('Error', "Decrypted ML-KEM private key is corrupted!");
                await new Promise(() => {});
                throw new Error('Decrypted ML-KEM private key is corrupted!');
              } else {
                //console.log(decrypted_private_key);

                const tagsRef = collection(db, `data/${user.email}/receivedFiles`);
                ...
                for (const doc of querySnapshot.docs) {
                  const data = doc.data() as EncryptedFileTag
                  ;
                  fileIndex++;
          
                  // Show pop-up for decrypting each record
                  showTwoLinedPopup(`Decrypting received tag N${fileIndex}/${querySnapshot.docs.length}`, "Please wait for a while");
                  ...
                  try {
                    ...
                    if (data.mlkemCiphertext) {
                      ...
                      if (encryptedKey.length > 1 && encryptedKey.length % 2 === 0) {
                        const decryptionKeySource = await decapsulateSecret(encryptedKey, decrypted_private_key);
                        if (data.encryptedTag) {
                          ...
                          if (encryptedFileTag.length > 1 && encryptedFileTag.length % 16 === 0) {
                            if (decryptionKeySource) {
                              const [decryptedWithStreamCipher, integrityCheckPassed, paddingValid] = await decryptStringWithTwoCiphersCBC(encryptedFileTag, decryptionKeySource, 125);
                          ...
                          ftitle = 'Missing encrypted tag.';
                          fdescription = 'Missing encrypted tag.';
                        }
                      } else {
                        ...
                        ftitle = 'Invalid ML-KEM ciphertext detected.';
                        fdescription = 'Invalid ML-KEM ciphertext detected.';
                      }
                    } else {
                      ...
                      ftitle = 'Missing ML-KEM ciphertext.';
                      fdescription = 'Missing ML-KEM ciphertext.';
                    }
              ...
              showTwoLinedPopup('Error', "Encrypted ML-KEM private key is corrupted!");
              await new Promise(() => {});
              throw new Error('Encrypted ML-KEM private key is corrupted!');
            }
...
const decapsulateSecret = async (ct: Uint8Array, skR: Uint8Array): Promise<Uint8Array | undefined> => {
  try {
      const recipient = new MlKem1024(); // Create a new recipient instance
      const ssR = await recipient.decap(ct, skR);
      //console.log("Recipient Shared Secret (ssR):", ssR);
      return ssR; // Return the recipient's shared secret
  } catch (err) {
...
function convertArrayIntoTheTag(uint8Array: Uint8Array): string {
  ...
  const handleDelete = async () => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        const docRef = doc(db, `data/${user.email}/receivedFiles`, currentRecord);
  ...
  return (
    ...
    <span style={{
            ...
            {t('received-files-tab')} {/* Translated text */}
          </span>
          {isPopupVisible && (
          ...
          <table style={{
            ...
            <tbody>
              {tags.map((tag) => {
                ...
                const userId = String.fromCharCode(...tag.tag.subarray(0, 16)); // Decode user ID
                const owner = decodeURIComponent(escape(userId)); // Decode if necessary
