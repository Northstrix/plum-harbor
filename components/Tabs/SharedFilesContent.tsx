              ...
              if (decryptedMetadataKey) {
                ...
                if (data.encryptedDescription && data.encryptedDescription.trim() !== '') {
                  if (data.encryptedDescription !== "Tm8gZGVzY3JpcHRpb24u") {
                    ...
                    decryptedDescription = t('no-description');
        ...
        const uploadFile = async (
          ...
          try {
            ...
            const newContainer: Container = {
              ...
              color: fileType ? fileType.color : '#3D4785',
              fileSize: `${fileSizeString}`,
              description: unencryptedDescription === "No description." ? t('no-description') : unencryptedDescription,
    ...
    return (
        <div className="flex flex-col items-center">
          <span style={{
            ...
            {t('shared-files-tab')} {/* Translated text */}
          </span>
          {descriptionPopup}
          {showProcessingPopup && (
                ...
                {containers.map(file => (
                  <BentoGridItem key={file.id}>
                    <BauhausFileCard
                        ...
                        fileName={file.title}
                        subMainText={file.description}
                        filledButtonInscription={t('get-tag')}
                        outlinedButtonInscription={t('all-options')}
                        onFilledButtonClick={handleGetTag}
                        onOutlinedButtonClick={handleOptionsClick}
                        filledChronicleButtonHoverColor={lightenColor(
                          fileTypeClassification.find(type =>
                              type.extensions.some(ext => file.title.toLowerCase().endsWith(ext))
                          )?.color || '#3D4785', 0.32
                        )}
                        oulinedChronicleButtonHoverColor={lightenColor(
                          fileTypeClassification.find(type =>
                              type.extensions.some(ext => file.title.toLowerCase().endsWith(ext))
                          )?.color || '#3D4785', 0.32
                        )}
